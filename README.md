...
![Simulation Overview](media/output.gif)
...
* [cite_start]**Mechanism:** Calculates the radar's altitude over time as it falls from an initial height of 200m [cite: 2] [cite_start]at a constant velocity of 13m/s[cite: 3]. It uses standard distance/time physics (`distance = velocity * time`) to subtract the fallen distance from the current height every frame.
* [cite_start]**Programming:** Introduces the frame timing logic, calculating the exact decimal fraction of a second between frames (for a 30 FPS video) [cite: 19, 20, 27] to ensure the simulation runs at real-world speed.

![Case 1 Demo](media/output%20(1).gif)
...
* [cite_start]**Mechanism:** Calculates the physical width of the ground the radar can see based on its 25° Field of View (FOV) [cite: 45] and current height.
* **Programming:** Introduces the `pixelsPerMeter` conversion ratio. This dynamically scales physical objects (like a 3-meter target) into a specific number of pixels so they can be drawn accurately on an 800x800 digital window. [cite_start]It also calculates the linear shift caused by the radar hanging at a 10° tilt[cite: 84].

![Case 2 Demo](media/output%20(2).gif)
...
![Case 3 Demo](media/output%20(3).gif)
...
![Case 4 Demo](media/output%20(4).gif)
...
![Case 5 Demo](media/output%20(5).gif)
...
![Case 6 Demo](media/output%20(6).gif)